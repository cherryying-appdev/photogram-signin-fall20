class UsersController < ApplicationController
  def authenticate
    ...
    if user == nil
      ...
      # (because we used has_secure_password library, we have a really great method called authenticate, will result in true or false)
      if user.authenticate(pw)
        session.store(:user_id, user.id)
        redirect_to("/", { :notice => "Welcome back, " + user.username + "!"})
      else
        # if not, redirect back to sign in form
        redirect_to("/user_sign_in", { :alert => "Nice try, but the password is incorrect"})
      end
    end
  end
